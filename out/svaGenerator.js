...
${propertyName}_a: assert property(${propertyName})
   ...
   */
    generateBidirectionalProperty(sourceNode, targetNode, timing, conditions, index) {
        const forwardProp = this.buildSVAProperty({ source: sourceNode.name, target: targetNode.name, operator: '->', isSharpLine: true }, sourceNode, targetNode, timing, conditions, index);
        const reverseProp = this.buildSVAProperty({ source: targetNode.name, target: sourceNode.name, operator: '->', isSharpLine: true }, targetNode, sourceNode, timing, conditions, index + 1000 // Offset to avoid naming conflicts
        );
