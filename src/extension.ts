...
import { WaveformToSVAGenerator, SVAGenerationResult } from './svaGenerator';
...
class WaveformRenderPanel {
   ...
   */
	...
	private _currentJsonData: any = null;
...
class SVAGeneratorPanel {
   ...
   */
	...
	public static createOrShow(extensionPath: string, jsonData?: any) {
		// If we already have a panel
		...
		const panel = vscode.window.createWebviewPanel(
			...
			"SystemVerilog Assertions: " + (activeEditor ? getFilename() : "Waveform"),
			{ preserveFocus: true, viewColumn: vscode.ViewColumn.Beside },
			{
				enableScripts: true,
				localResourceRoots: [
	...
	private constructor(panel: vscode.WebviewPanel, extensionPath: string, initialJsonData?: any) {
		this._panel = panel;
		this._extensionPath = extensionPath;
	...
	private _updateWithJsonData(jsonData: any) {
		try {
			const svaCode = this._generateSVAFromJSON(jsonData);
	...
	private _analyzeModuleRequirements(nodeMap: Map<string, NodeInfo>, _edges: EdgeInfo[]): any {
	...
	private _analyzeIndividualWavePattern(wavePattern: string, _dataArray: string[]): any {
		const analysis = {
	...
	private _generateClockAssertion(signalName: string, _wave: string): string {
	...
	private _generateReport(_svaContent: string): string {
 ...
 */
async function handleSVAGenerationResult(result: SVAGenerationResult, sourceFileName: string): Promise<void> {
 ...
 */
async function saveSVAToFile(result: SVAGenerationResult, sourceFileName: string): Promise<void> {
 ...
 */
function generateCompleteSystemVerilogModule(result: SVAGenerationResult, _moduleName: string): string {
